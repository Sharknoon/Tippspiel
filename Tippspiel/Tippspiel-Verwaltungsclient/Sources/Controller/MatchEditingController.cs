...
namespace Tippspiel_Verwaltungsclient.Sources.Controller
{
    public class MatchEditingController
    {
        ...
        public static void StartAsNewMatch(int seasonId, int matchDay)
        {
            ...
            MatchEditingWindow = new MatchEditingWindow {Match = match};
        ...
        {
            var allTeams = Service.GetAllTeams().ToList()
                .Where(team => team.SeasonIDs.Contains(MatchEditingWindow.CurrentSeason.Id)).ToList();
            ...
            {
                MatchEditingWindow.CurrentHomeTeam =
                    allTeams.Find(team => team.Id.Equals(MatchEditingWindow.Match.HomeTeamId));
                MatchEditingWindow.CurrentAwayTeam =
                    allTeams.Find(team => team.Id.Equals(MatchEditingWindow.Match.AwayTeamId));
            }
        ...
        public static void EditingFinished()
        {
            ...
            var errors = IsNewMatch
                ? Service.CreateMatch(MatchEditingWindow.Match)
                : Service.EditMatch(MatchEditingWindow.Match);
