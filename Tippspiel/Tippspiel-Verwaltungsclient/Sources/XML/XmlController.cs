...
namespace Tippspiel_Verwaltungsclient.Sources.XML
{
    public class XmlController
    {
        ...
        {
            var matches = new List<MatchMessage>();
            ...
            if (matchDay == null)
                ...
                {
                    var homeTeamList = Service.GetTeamByName(xmlMatch.Heim.Name).ToList();
                    if (homeTeamList.IsEmpty())
                    {
                        OnError("Die Heimmannschaft " + xmlMatch.Heim.Name + " existiert nicht!");
                        continue;
                    }
                    var awayTeamList = Service.GetTeamByName(xmlMatch.Auswaerts.Name).ToList();
