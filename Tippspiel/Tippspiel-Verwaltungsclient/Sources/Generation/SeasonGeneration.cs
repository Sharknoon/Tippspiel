...
namespace Tippspiel_Verwaltungsclient.Sources.Generation
{
    public class SeasonGeneration
    {
        ...
        public static void GenerateSeason(DateTime date, SeasonMessage season)
        {
            ...
            var completeSeason = AddSecondRound(firstRound);
            var waiting = new WaitingWindow();
            waiting.Show();
            var errors = Service.CreateMatches(completeSeason.Values.SelectMany(mday => mday).ToArray());
            waiting.Close();
        ...
        {
            var completeSeason = new Dictionary<int, List<MatchMessage>>();
            foreach (var kvpair in firstRound)
                completeSeason.Add(kvpair.Key.DeepClone(), kvpair.Value.DeepClone());
            foreach (var matchDay in firstRound
            ) //swap home and awayteam, move date 17 weeks forward, add 17 to matchdate
            {
                var swappedMatches = new List<MatchMessage>();
                foreach (var match in matchDay.Value)
                {
                    var oldHomeTeam = match.HomeTeamId;
        ...
        public static MatchMessage GenerateMatch(TeamMessage teamA, TeamMessage teamB, int matchDay,
            int matchOfMatchDay, int seasonId)
        {
