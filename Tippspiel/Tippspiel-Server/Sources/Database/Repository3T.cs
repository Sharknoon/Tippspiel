...
{
    public class Repository3T<T, TU, TV> where T : class where TU : class where TV : class
    {
        ...
        public List<T> GetByExpressionOnReference(Expression<Func<T, TV>> reference)
        {
            return GetByExpressionOnReference(null, reference, null);
        }

        public List<T> GetByExpressionOnReference(Expression<Func<T, TV>> reference,
            Expression<Func<TV, bool>> optionalJoinExpression)
        {
            return GetByExpressionOnReference(null, reference, optionalJoinExpression);
        }

        public List<T> GetByExpressionOnReference(Expression<Func<T, bool>> optionalMainExpression,
            Expression<Func<T, TV>> reference, Expression<Func<TV, bool>> optionalJoinExpression)
        {
            using (var session = NHibernateHelper.OpenSession())
            {
                var queryOver = session.QueryOver<T>();
        ...
        public List<T> GetByExpressionOnReference(Expression<Func<T, IEnumerable<TV>>> reference)
        {
            return GetByExpressionOnReference(null, reference, null);
        }
        ...
        public List<T> GetByExpressionOnReference(Expression<Func<T, bool>> optionalMainExpression,
            Expression<Func<T, IEnumerable<TV>>> reference, Expression<Func<TV, bool>> optionalJoinExpression)
        {
            using (var session = NHibernateHelper.OpenSession())
            {
                var queryOver = session.QueryOver<T>();
